...
\begin{center}
\textit{Manual oficial para las compilaciones 1.0 de MLR-X (Linux, macOS, Windows GUI y Windows GUI+CLI). El código fuente `MLRX\_source.py` sirve solo como referencia técnica; los usuarios finales deben operar exclusivamente con los ejecutables suministrados.}
\end{center}
...
\begin{itemize}
  \item \textbf{Builds disponibles}: binarios autocontenidos para Linux, macOS (GUI), Windows GUI y Windows GUI+CLI.
  \item \textbf{Requisitos del sistema}: Windows, Linux o macOS de 64 bits. No es necesario instalar Python ni dependencias científicas; todo viene incluido en las compilaciones.
  \item \textbf{Versión en consola}: el flag \verb|--version| imprime \texttt{MLR-X version 1.0} en las builds con CLI (Linux y Windows GUI+CLI).
\end{itemize}

% =========================================================
\section{Requisitos e instalación}
Las compilaciones distribuidas son autocontenidas y no requieren instalación de Python ni de bibliotecas adicionales. Seleccione la build adecuada y siga estos pasos:

\subsection{Instalación desde binarios}
\begin{itemize}
  \item \textbf{Linux}: descargar el binario, moverlo a una carpeta de trabajo y marcarlo como ejecutable si es necesario (\texttt{chmod +x MLR-X}).
  \item \textbf{Windows GUI}: ejecutar el instalador y abrir el acceso directo generado para la interfaz gráfica.
  \item \textbf{Windows GUI+CLI}: ejecutar el instalador; se incluye acceso directo para la GUI y el ejecutable CLI en la ruta de instalación.
  \item \textbf{macOS GUI}: abrir el paquete firmado y arrastrar la app a \texttt{/Applications}.
\end{itemize}

\subsection{Notas sobre el código fuente}
El archivo \texttt{MLRX\_source.py} documenta la lógica interna, pero no está pensado para instalación directa. Para soporte y estabilidad, utilice únicamente las builds oficiales citadas arriba.

% =========================================================
\section{Inicio rápido}
\subsection{Abrir la interfaz gráfica}
\begin{itemize}
  \item \textbf{Windows GUI / macOS GUI}: ejecutar el acceso directo o la app \emph{MLR-X}. No se requieren parámetros adicionales.
  \item \textbf{Linux}: desde terminal, ubicarse en la carpeta del binario y ejecutar \verb|./MLR-X|.
\end{itemize}
La aplicación precarga las bibliotecas pesadas en segundo plano para mejorar el tiempo de respuesta inicial.

\subsection{Ejecutar el modo CLI}
Disponible únicamente en las compilaciones con CLI (Linux y Windows GUI+CLI). Ejemplos:
\begin{lstlisting}[style=mlrx]
./MLR-X --version          # Linux
MLR-X.exe --version        # Windows GUI+CLI
./MLR-X ruta/al/archivo.conf
\end{lstlisting}
El comando con archivo de configuración carga los parámetros definidos y ejecuta el flujo completo en consola.
...
\begin{itemize}
  \item \textbf{La GUI no abre}: reinstalar la build correspondiente y verificar que el binario no esté bloqueado por el antivirus.
